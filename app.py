...
# =============================================================================
# REAL-TIME LIVE PRICE & DATA FETCHER (CLOUDS-SAFE)
# =============================================================================
@st.cache_data(ttl=3, show_spinner=False)
def fetch_base_market_data(pair: str, limit: int = 100):
    ...
    try:
        ...
        if isinstance(data, list) and len(data) > 0:
            ...
            return df[["open", "high", "low", "close", "volume"]], "Binance Live"
    except Exception:
        pass
        
    # Fallback to CoinGecko if Binance blocked
    try:
        ...
        resp = requests.get(cg_url, params={"vs_currency": "usd", "days": "1"}, timeout=4)
        ...
        if prices:
            ...
            df["volume"] = 1000.0
            return df.tail(limit), "CoinGecko Live"
    except Exception:
        pass

    return pd.DataFrame(), "Error"

# Fetch exact current live price ticker to keep it real-time every second
def get_live_ticker_price(pair: str):
    symbol = pair.replace("/", "").upper()
    try:
...
st.markdown("## 📊 Pro Live Trading Terminal (1s Refresh)")

try:
    raw_df, source = fetch_base_market_data(pair)
    
    if raw_df is None or raw_df.empty:
        st.warning("⚠️ மார்க்கெட் டேட்டா கிடைப்பதில் சிறு தாமதம்... அடுத்த விநாடியில் சரியாகிவிடும்.")
        st.stop()
        
    # Get exact real-time live tick price and update the last candle close dynamically
    live_price = get_live_ticker_price(pair)
    if live_price and not raw_df.empty:
    ...
    if df.empty or len(df) < 10:
        st.warning("⚠️ போதுமான டேட்டா கிடைக்கவில்லை...")
        st.stop()
    ...
    with m6: st.markdown(f"<div class='metric-card'><div class='metric-label'>Source</div><div class='metric-value' style='font-size:12px;color:#0ecb81;'>{source} (1s)</div></div>", unsafe_allow_html=True)
