...
# Static fallback pair list, used if live market-loading for an exchange
...
# When a chosen exchange fails outright (timeout / geo-block / bad symbol),
# these are tried next, in order, before falling back to CoinGecko.
...
def generate_signal(df: pd.DataFrame) -> dict:
    ...
    # Volume confirmation (amplifies existing lean, doesn't set direction alone)
...
def render_upgrade_modal():
    ...
    st.write("Unlock every exchange and pair, intraday timeframes, and the full indicator stack.")
    col1, col2 = st.columns(2)
    with col1:
        st.markdown("**Free**")
        st.markdown(
            "- Binance & Kraken only\n- 2 pairs (BTC, ETH)\n- 1h / 4h / 1d timeframes\n"
            "- Bollinger Bands, EMA, Volume, RSI\n- Basic Buy/Sell/Hold signal"
        )
    with col2:
        st.markdown("**Premium Pro — $19/mo**")
        st.markdown(
            "- All 7 exchanges\n- Full pair search per exchange\n- All timeframes incl. 5m/15m\n"
            "- + MACD, KDJ, Stochastic RSI\n- Full confluence breakdown + confidence score\n- Order book + raw data export"
        )
    ...
    if st.button(f"Pay with {pay_provider} (Demo)", type="primary"):
        ...
        # Replace with a real server-side call, e.g.:
        #
        # Stripe:
        #   import stripe
        ...
        #
        # NEVER put live secret keys in client-side code. Use st.secrets and
        # a backend/webhook to verify payment before granting access.
...
def main_app():
    ...
    is_premium = tier == "Premium Pro"
    badge_class = "premium-badge" if is_premium else "free-badge"
    ...
    available_exchanges = EXCHANGES if is_premium else FREE_EXCHANGES
    exchange_id = st.sidebar.selectbox("Exchange", available_exchanges)
    if not is_premium:
        st.sidebar.caption(f"🔒 {len(EXCHANGES) - len(FREE_EXCHANGES)} more exchanges on Premium Pro")

    live_symbols = load_markets_for_exchange(exchange_id) if is_premium else []
    if is_premium and live_symbols:
        pair_pool = live_symbols
    else:
        pair_pool = FALLBACK_PAIRS if is_premium else FREE_PAIRS
    pair = st.sidebar.selectbox(f"Trading Pair ({len(pair_pool)} available)", pair_pool)
    if not is_premium:
        st.sidebar.caption("🔒 Full pair search unlocked on Premium Pro")

    available_tfs = TIMEFRAMES if is_premium else FREE_TIMEFRAMES
    ...
    if is_premium:
        ...
        st.sidebar.markdown("MACD, KDJ, Stochastic RSI <span class='lock-badge'>Premium Pro</span>", unsafe_allow_html=True)
    ...
    # ---- Top metric row (24h-style stats sized to the chosen timeframe) ----
    ...
    with left_col:
        ...
        if is_premium:
            ...
            st.info("Basic signal shown. Upgrade to Premium Pro for the full breakdown, confidence score, and order book.")
        ...
        if is_premium:
            ...
            st.caption("🔒 Order book depth is a Premium Pro feature.")
    ...
    st.markdown(
        ...
        do not guarantee future results. Authentication and payments on this demo are simulated. Always
