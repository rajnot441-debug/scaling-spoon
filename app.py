...
def build_chart(df: pd.DataFrame, pair: str) -> go.Figure:
    ...
    fig.update_layout(
        ...
        # Disabling dragmode stops a single-finger touch (or mouse drag) from
        # panning/zooming the chart. Pinch-to-zoom is a native browser/touch
        # gesture handled outside dragmode, so two-finger zoom still works.
        dragmode=False,
    )
...
def main_app():
    ...
    with chart_col:
        chart_config = {
            # Mouse-wheel / trackpad scroll zoom off — this is the interaction
            # that made a single touch/scroll feel overly sensitive.
            "scrollZoom": False,
            # Hide the modebar so there's no stray zoom/pan/select tool active.
            "displayModeBar": False,
            # Double-tap/click still resets the view, but doesn't autoscale
            # into an unexpected zoom level.
            "doubleClick": "reset",
            "displaylogo": False,
        }
        st.plotly_chart(build_chart(df, pair), use_container_width=True, config=chart_config)
