...
public-API fallback if every exchange attempt fails. The dashboard
auto-refreshes every 5-10 seconds via streamlit-autorefresh, re-polling on
each cycle (cache-aware — see fetch_ohlcv). Worth being precise: this is
REST polling, not a websocket tick stream, so updates land in 5-10s
increments rather than trade-by-trade. True tick-level streaming would
need an exchange websocket feed (e.g. ccxt.pro), which is out of scope
here. Includes a simulated login flow and a Free vs Pro tier gate unlocked
by a manually-distributed activation key (see PRO SUBSCRIPTION section for
...
<style>
    ...
    * { font-variant-numeric: tabular-nums; }
    ...
    .live-dot {
        width: 9px; height: 9px; border-radius: 50%; background: #0ecb81;
        animation: live-pulse 1.4s ease-in-out infinite;
    }
    .live-dot.paused { background: #5b6472; animation: none; }
    @keyframes live-pulse {
        0%   { box-shadow: 0 0 0 0 rgba(14,203,129,0.6); }
...
def main_app():
    ...
    live_status_placeholder = st.empty()  # filled in after refresh_interval/live_enabled are known below

    # ---- Sidebar: live auto-refresh ----
    st.sidebar.header("🔴 Live Mode")
    live_enabled = st.sidebar.checkbox("Enable Live Auto-Refresh", value=True)
    if is_pro:
        refresh_interval = st.sidebar.slider(
            "Refresh interval (seconds)", min_value=5, max_value=10, value=5,
            disabled=not live_enabled,
        )
    else:
        refresh_interval = 10
        st.sidebar.caption("Free tier refreshes every 10s. Pro can go as low as 5s.")
    ...
    if live_enabled:
        if st_autorefresh is not None:
            ...
            live_enabled = False
            st.sidebar.warning(
                "`streamlit-autorefresh` isn't installed, so auto-refresh is off. "
                "Run `pip install streamlit-autorefresh` to enable it, or use the "
                "manual Refresh button below meanwhile."
            )
    ...
    st.sidebar.divider()
    st.sidebar.caption(f"Preferred source: {exchange_id} → auto-fallback chain → CoinGecko · cached 12s")
    ...
    dot_cls = "live-dot" if live_enabled else "live-dot paused"
    live_text = f"LIVE · polling every {refresh_interval}s" if live_enabled else "PAUSED · manual refresh only"
    last_candle_time = df.index[-1].strftime("%H:%M:%S")
    live_status_placeholder.markdown(
        f"<div class='live-bar'><span class='{dot_cls}'></span>{live_text} · "
        f"last candle {last_candle_time} · checked {datetime.now().strftime('%H:%M:%S')}</div>",
        unsafe_allow_html=True,
    )
