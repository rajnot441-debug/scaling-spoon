...
def fetch_live_ohlcv(pair: str, timeframe: str, limit: int = 150):
    ...
    resp = requests.get(url, params={"vs_currency": "usd", "days": "1"}, timeout=5)
    ...
    df["volume"] = 1000.0
    return df.tail(limit), "CoinGecko Fallback"
...
def generate_signals(df: pd.DataFrame):
    ...
    if last["close"] > last["ema_50"]: score += 1
    else: score -= 1
...
try:
    ...
    # 1. Top Metrics Header (No Subplots - All indicators shown here)
    ...
    # Layout: Left Signal Panel, Right Main Chart (Overlays Only)
    ...
    with left_col:
        ...
        st.write("")
        st.info("💡 **Live Analysis Status:**\n- Candlestick updates every 1 second automatically.\n- Indicators (Bollinger, EMA, MA) overlaid on main chart.\n- Zero subplots to keep UI clean.")

    with right_col:
        # Plotly Candlestick Chart with On-Chart Overlays (Bollinger, EMA, MA)
        fig = go.Figure()

        # Bollinger Bands Fill
        fig.add_trace(go.Scatter(x=df.index, y=df["bb_upper"], name="BB Upper", line=dict(color="rgba(100,150,255,0.3)", width=1), hoverinfo='skip'))
        fig.add_trace(go.Scatter(x=df.index, y=df["bb_lower"], name="BB Lower", line=dict(color="rgba(100,150,255,0.3)", width=1), fill='tonexty', fillcolor='rgba(100,150,255,0.05)', hoverinfo='skip'))
        ...
        fig.add_trace(go.Scatter(x=df.index, y=df["ma_20"], name="MA 20", line=dict(color="#f0b90b", width=1.2)))
        fig.add_trace(go.Scatter(x=df.index, y=df["ema_20"], name="EMA 20", line=dict(color="#3498db", width=1.2)))
        fig.add_trace(go.Scatter(x=df.index, y=df["ema_50"], name="EMA 50", line=dict(color="#9b59b6", width=1.5)))

        # Chart Layout configuration (Strict Double-finger zoom restriction control via config)
        fig.update_layout(
            height=500,
        ...
        # Config to disable single finger dragging distortion / restrict zoom behavior cleanly
        chart_config = {
            "scrollZoom": True,
